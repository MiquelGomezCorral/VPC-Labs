...
def load_car_data(CONFIG: Configuration, use_transforms=True):
    """
    Load and preprocess the car classification dataset based on the provided configuration.
    Args:
 ...
 (bool): Whether to apply data augmentations (default: True).
    ...
    # Convert labels from {1, 20} to {0, 19}
    y_train = y_train - 1 
    y_test = y_test - 1  
    ...
    train_dataset = CarDataset(x_train, y_train, transform=train_transform)
    test_dataset  = CarDataset(x_test,  y_test, transform=test_transform)
